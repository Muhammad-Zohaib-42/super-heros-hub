...
const Header = () => {
  ...
  const {isDark, setIsDark, setSearchHero, setSearchError} = useContext(HeroContext)!
  ...
  return (
    <header className="h-15 border-b border-b-slate-200 dark:border-b-slate-700 flex items-center justify-between px-5 dark:bg-slate-800 dark:text-slate-200 shrink-0">
        <div>
            <form onSubmit={handleSubmit} className="border border-slate-200 dark:border-slate-700 focus-within:border-slate-300 dark:focus-within:border-slate-600 h-10 w-87.5 flex rounded-full relative">
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="border-none outline-none pl-4 pr-12 py-2 h-full grow text-slate-700 dark:text-slate-200 font-normal" type="text" placeholder="Search superhero" />
